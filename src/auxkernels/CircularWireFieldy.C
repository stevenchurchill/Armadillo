...
CircularWireFieldy::CircularWireFieldy(const InputParameters & parameters) :
...
Real
CircularWireFieldy::computeValue()
...
/// Cartesian components -- likely a typo or something in the Nasa paper... results are not correct.
  ...
  /// Note that we don't have C++17 support (comm. w/ D. Schwen), so the coksqp_ellint_1 and 2 functions don't have headers. We can use asymptotic expansions instead.
  ...
  sum += (C * y * z / (2.0 * alpsq * std::pow(betsq, 0.5) * std::pow(rho, 2.0)) ) * (( std::pow(_a, 2.0) + std::pow(r, 2.0) ) * asymptotK - alpsq * asymptotE  );
  return sum;
}
