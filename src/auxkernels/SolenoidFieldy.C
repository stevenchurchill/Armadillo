...
SolenoidFieldy::SolenoidFieldy(const InputParameters & parameters) :
  ...
  for (unsigned int j = 1; j < _N; ++j)
   {
     ...
     sum1 += (C * y * z / (2.0 * alpsq * std::pow(betsq, 0.5) * std::pow(rho, 2.0)) ) * (( std::pow(_a, 2.0) + std::pow(r, 2.0) ) * asymptotK - alpsq * asymptotE  );
   }
